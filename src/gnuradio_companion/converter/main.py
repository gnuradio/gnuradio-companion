...
class Converter(object):
    def __init__(
        self,
        search_path: str,
        output_dir: str = os.path.join(get_state_directory(), Constants.GRC_SUBDIR),
    ):
    ...
    def load_block_xml(self, xml_file, force=False):
        ...
        if any(part in xml_file for part in excludes) and not force:
            logger.warning("Skipping {} because name is blacklisted!".format(xml_file))
            return False
        elif any(part in xml_file for part in excludes) and force:
            logger.warning("Forcing conversion of blacklisted file: {}".format(xml_file))
