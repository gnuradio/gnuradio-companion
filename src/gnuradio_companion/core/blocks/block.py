...
class Block(Element):
    ...
    def get_cpp_value(self, pyval, vtype: typing.Optional[type] = None) -> str:
        ...
        if vtype == int or vtype == float:
            ...
            elements = [
                str(self.get_cpp_value(element, item_type)) for element in pyval
            ]
            ...
            entries = [
                "{"
                + self.get_cpp_value(key, key_type)
                + ", "
                + self.get_cpp_value(val, val_type)
                + "}"
    ...
    def is_deprecated(self):
        ...
        try:
            return self.flags.deprecated or any(
                "deprecated".casefold() in cat.casefold() for cat in self.category
            )
