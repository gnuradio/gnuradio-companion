...
class Param(Element):
    ...
    def _init_options(self, values, labels, attributes):
        ...
        for i, option in enumerate(values):
            ...
            options.attributes[option] = {
                attrib: values[i] for attrib, values in attributes.items()
            }
    ...
    def parse_gui_hint(self, expr: str) -> str:
        ...
        def parse_pos():
            ...
            if (row < 0) or (col < 0):
                self.add_error_message(
                    "Invalid GUI Hint entered: {e!r} (non-negative rows/cols only).".format(
                        e=e
                    )
                )
