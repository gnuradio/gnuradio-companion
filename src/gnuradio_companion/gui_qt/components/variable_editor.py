...
class VariableEditor(QDockWidget, base.Component):
    ...
    def __init__(self):
        ...
        self.app.registerDockWidget(
            self, location=self.settings.window.VARIABLE_EDITOR_DOCK_LOCATION
        )
        self.currently_rebuilding = False
        if not self.qsettings.value(
            "appearance/display_variable_editor", True, type=bool
        ):
            self.hide()
    ...
    def _rebuild(self):
        ...
        for block in self._imports:
            ...
            if block.enabled:
                ...
                import_.setForeground(
                    0, self._tree.palette().color(self.palette().WindowText)
                )
                import_.setForeground(
                    1, self._tree.palette().color(self.palette().WindowText)
                )
            else:
                import_.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
                import_.setForeground(
                    0,
                    self._tree.palette().color(
    ...
    def handle_action(self, action):
        ...
        if (
            self._tree.currentItem().type() == 2
        ):  # Import or Variable header line was selected
            return

        self.scene.clearSelection()
        if self._tree.currentItem().type() == 0:
            to_handle = self.scene.core.blocks.index(
                self._imports[self._tree.currentIndex().row()]
            )
        else:
            to_handle = self.scene.core.blocks.index(
                self._variables[self._tree.currentIndex().row()]
            )
...
class VariableEditorContextMenu(QMenu):
    def __init__(self, var_edit: VariableEditor):
        ...
        self.imports.triggered.connect(
            lambda: var_edit.handle_action(VariableEditorAction.ADD_IMPORT)
        )
        self.addAction(self.imports)

        self.variables = QAction(_("variable_editor_add_variable"), self)
        self.variables.triggered.connect(
            lambda: var_edit.handle_action(VariableEditorAction.ADD_VARIABLE)
        )
        ...
        self.enable.triggered.connect(
            lambda: var_edit.handle_action(VariableEditorAction.ENABLE_BLOCK)
        )
        self.addAction(self.enable)

        self.disable = QAction(_("variable_editor_disable"), self)
        self.disable.triggered.connect(
            lambda: var_edit.handle_action(VariableEditorAction.DISABLE_BLOCK)
        )
        ...
        self.delete.triggered.connect(
            lambda: var_edit.handle_action(VariableEditorAction.DELETE_BLOCK)
        )
        ...
        self.properties.triggered.connect(
            lambda: var_edit.handle_action(VariableEditorAction.OPEN_PROPERTIES)
        )
